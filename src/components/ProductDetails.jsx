        ...
        <div className="flex flex-col items-center justify-center mb-8">
          <div>
            <img
              ...
              alt={product.title}
              className="w-full max-w-full max-h-96 object-cover rounded-lg border border-gray-300 mb-6"
            />
          </div>
          {product?.description && (
            <div className="text-center text-lg sm:text-xl font-semibold text-gray-700">
              {product.description}
            </div>
          )}
        </div>
        ...
        {product?.images && (
          <div className="flex flex-col sm:flex-row justify-center mb-8">
            {product.images.map((x, i) => (
              <div className="flex flex-col items-center mb-6 sm:mb-0 sm:mr-6" key={i}>
                <img
                  src={x.url}
                  alt={product.title}
                  className="w-full sm:w-64 max-h-64 object-cover rounded-lg border border-gray-300 mb-4"
                />
                <div className="text-center text-lg font-semibold">{x.text}</div>
              </div>
            ))}
          </div>
        )}
          ...
          {details.map(
            (detail, index) =>
              detail.heading && (
                ...
                >
                  <h2 className="text-xl sm:text-2xl font-semibold text-blue-600 mb-2">{detail.heading}</h2>
                  <p className="text-base sm:text-lg text-gray-600 leading-relaxed">{detail.text}</p>
                </div>
              )
          )}
        </div>
